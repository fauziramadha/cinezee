...
const GENRE_NAMES: Record<string, string> = {
  ...
  "asian": "Asian", "biography": "Biography", "indian": "Indian", "comedy": "Comedy",
  "crime": "Crime", "documentary": "Documentary", "drama": "Drama", "family": "Family",
  "fantasy": "Fantasy", "film-noir": "Film Noir", "game-show": "Game Show", "history": "History",
  "horror": "Horror", "music": "Music", "musical": "Musical", "mystery": "Mystery",
  "news": "News", "reality-tv": "Reality TV", "romance": "Romance", "sci-fi": "Sci-Fi",
  "short": "Short", "sport": "Sport", "specials": "Specials", "stand-up": "Stand Up",
  "talk-show": "Talk Show", "thriller": "Thriller", "war": "War", "western": "Western",
};
...
export async function GET() {
  ...
  if (cached) {
    try { return NextResponse.json({ ...JSON.parse(cached.body), _cache: "HIT" }); } catch {}
  }

  const cookieAccount = await dbCinemacity.getActiveCookies();
  if (!cookieAccount) return NextResponse.json({ error: "No active cinemacity cookie account" }, { status: 503 });

  let response: Response;
  try {
    response = await fetch(`${CINEMACITY_BASE}/`, {
      headers: {
        "User-Agent": DEFAULT_UA, "Accept": "text/html,*/*",
        "Cookie": cookiesToHeader(cookieAccount.cookies), "Referer": CINEMACITY_BASE + "/",
      },
      redirect: "follow",
    });
  } catch (error) {
  ...
  try {
    await dbCinemacity.setCache({
      cache_key: cacheKey, endpoint: "/cinemacity:genres:list", method: "GET",
      status_code: 200, body: JSON.stringify(responseData),
      content_type: "application/json", ttl_seconds: 86400,
    });
  } catch {}
