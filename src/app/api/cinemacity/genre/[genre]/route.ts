...
export async function GET(
  ...
  const responseData = { genre, page, movies, count: movies.length, source: "cinemacity.cc" };

  try {
    await dbCinemacity.setCache({
      cache_key: cacheKey, endpoint: `/cinemacity:genre:${genre}:${page}`, method: "GET",
      status_code: 200, body: JSON.stringify(responseData),
      content_type: "application/json", ttl_seconds: 1800,
    });
  } catch {}
