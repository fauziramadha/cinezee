          ...
          {loading ? (
            ...
            <div className="fade-in pb-16">
              ...
              <div className="flex flex-wrap items-center gap-2 border-b border-border bg-card/50 px-4 py-3 sm:gap-3 sm:px-6 md:px-8">
                ...
                <Button size="icon" variant="outline" aria-label="Share" onClick={() => {  const shareUrl = `${window.location.origin}/?${selectedMedia.type}=${selectedMedia.id}`;    if (navigator.share) {    navigator.share({       title: title,       text: `Tonton ${title} di CineStream!`,       url: shareUrl     }).catch(() => {});  } else {    navigator.clipboard.writeText(shareUrl);    toast.success("Link film disalin ke clipboard!");  }}}><Share2 className="h-4 w-4" /></Button>
              </div>
                ...
                <div className="border-b border-border bg-card/30 px-4 py-3 sm:px-6 md:px-8">
                  <div className="mb-3 flex items-center gap-2">
                    <Select value={String(season)} onValueChange={(v) => { setSeason(parseInt(v, 10)); setEpisode(1); }}>
                      ...
                      <SelectContent>{detail.seasons.filter((s) => s.season_number > 0).map((s) => (<SelectItem key={s.id} value={String(s.season_number)}>S{s.season_number}</SelectItem>))}</SelectContent>
                    </Select>
              ...
              <div className="grid gap-4 p-4 sm:gap-6 sm:p-6 md:grid-cols-3 md:p-8">
                <div className="min-w-0 md:col-span-2">
                  ...
                  <p className="break-words text-xs leading-relaxed text-foreground/90 sm:text-sm md:text-base">{detail.overview || "No overview available."}</p>
                  ...
                  {cast.length > 0 && (
                    <div className="mt-4 sm:mt-6">
                      ...
                      <div className="grid grid-cols-2 gap-2 overflow-hidden sm:grid-cols-3 sm:gap-3 lg:grid-cols-4">
                        {cast.map((p) => (<div key={p.id} className="flex min-w-0 items-center gap-2"><div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-full bg-muted sm:h-12 sm:w-12">{p.profile_path && <Image src={getImageUrl(p.profile_path, "w185")} alt={p.name} fill sizes="48px" className="object-cover" unoptimized />}</div><div className="min-w-0 flex-1 overflow-hidden"><p className="truncate text-xs font-medium sm:text-sm">{p.name}</p><p className="truncate text-[10px] text-muted-foreground sm:text-xs">{p.character}</p></div></div>))}
                      </div>
                    </div>
                  )}
                ...
                <div className="min-w-0 space-y-3 sm:space-y-4">
                  {(director || creator) && <div className="overflow-hidden"><h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{isTV ? "Creator" : "Director"}</h4><p className="mt-0.5 break-words text-xs sm:text-sm">{director || creator}</p></div>}
                  ...
                  {detail.spoken_languages && detail.spoken_languages.length > 0 && <div className="overflow-hidden"><h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Languages</h4><p className="mt-0.5 break-words text-xs sm:text-sm">{detail.spoken_languages.map((l) => l.english_name).join(", ")}</p></div>}
                  {detail.production_companies && detail.production_companies.length > 0 && <div className="overflow-hidden"><h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Production</h4><p className="mt-0.5 break-words text-xs sm:text-sm">{detail.production_companies.map((c) => c.name).join(", ")}</p></div>}
                </div>
              </div>
  ...
  return (
    <div className={level > 0 ? "ml-6 border-l border-border pl-3" : ""}>
      <div className="flex gap-2">
        ...
        <div className="min-w-0 flex-1">
          <div className="rounded-lg bg-muted/40 px-3 py-2"><div className="flex items-center justify-between gap-2"><span className="text-xs font-semibold">{comment.userName || "Anonymous"}</span><span className="text-[10px] text-muted-foreground">{timeAgo}</span></div><p className="mt-1 break-words text-xs leading-relaxed sm:text-sm">{comment.content}</p></div>
