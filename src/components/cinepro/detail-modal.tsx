...
export function DetailModal() {
  ...
  const handlePostComment = async () => {
    ...
    try { const res = await fetch("/api/comments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mediaId: selectedMedia.id, mediaType: selectedMedia.type, content: commentText }) });
  ...
  const handlePostReply = async (parentId: string) => {
    ...
    setCommentLoading(true);
    try { const res = await fetch("/api/comments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mediaId: selectedMedia.id, mediaType: selectedMedia.type, content: replyText, parentId }) });
        ...
        <ScrollArea className="max-h-[95vh]">
          {loading ? (
            ...
            <div className="fade-in pb-20">
              ...
              <div className="relative h-[25vh] min-h-[160px] w-full overflow-hidden bg-muted sm:h-[35vh] md:aspect-video md:h-auto">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-6 md:p-8">
                  ...
                  {detail.tagline && <p className="mt-0.5 text-[10px] italic text-white/80 sm:text-sm">&quot;{detail.tagline}&quot;</p>}
                </div>
              </div>

              {/* Action bar */}
              <div className="flex flex-wrap items-center gap-2 border-b border-border bg-card/50 px-4 py-3 sm:gap-3 sm:px-6 md:px-8">
                <Button size="sm" onClick={handleOpen} className="gap-2 bg-primary text-primary-foreground hover:bg-primary/90 sm:size-lg"><Play className="h-4 w-4 fill-current" /><span className="text-xs sm:text-sm">Play Now</span></Button>
                ...
                <div className="border-b border-border bg-card/30 px-4 py-3 sm:px-6 md:px-8">
                  ...
                  <div className="mb-3 flex items-center gap-2">
                    <Select value={String(season)} onValueChange={(v) => { setSeason(parseInt(v, 10)); setEpisode(1); }}>
                      <SelectTrigger className="h-8 flex-1 text-xs sm:w-32"><SelectValue /></SelectTrigger>
                      <SelectContent>{detail.seasons.filter((s) => s.season_number > 0).map((s) => (<SelectItem key={s.id} value={String(s.season_number)}>Season {s.season_number}</SelectItem>))}</SelectContent>
                    </Select>
                    <div className="flex gap-1">
                      <Button size="icon" variant="ghost" className="h-8 w-8" disabled={episode <= 1} onClick={() => setEpisode(Math.max(1, episode - 1))}><ChevronLeft className="h-4 w-4" /></Button>
                      <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => setEpisode(episode + 1)}><ChevronRight className="h-4 w-4" /></Button>
                    </div>
                  </div>

                  {/* Episode thumbnails - horizontal scroll, clickable */}
                  {episodesLoading ? (
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
                      {episodes.map((ep) => (
                        <button key={ep.episodeNumber} onClick={() => setEpisode(ep.episodeNumber)} className={`relative aspect-video w-32 shrink-0 overflow-hidden rounded-lg border-2 transition-all sm:w-40 ${episode === ep.episodeNumber ? "border-primary" : "border-transparent hover:border-border"}`}>
                          {ep.stillPath ? <Image src={getImageUrl(ep.stillPath, "w300")} alt={`Ep ${ep.episodeNumber}`} fill sizes="160px" className="object-cover" unoptimized /> : <div className="flex h-full items-center justify-center bg-muted"><span className="text-[10px] text-muted-foreground">No Image</span></div>}
                          <div className="absolute inset-0 bg-gradient-to-t from-black/90 to-transparent" />
                          <div className="absolute bottom-1 left-1 right-1">
                            <p className="truncate text-[9px] font-bold text-white sm:text-[10px]">EP {ep.episodeNumber}</p>
                            <p className="truncate text-[8px] text-white/70 sm:text-[9px]">{ep.name}</p>
                          </div>
                          {episode === ep.episodeNumber && <div className="absolute inset-0 flex items-center justify-center bg-primary/20"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary"><Play className="h-3 w-3 fill-white text-white" /></div></div>}
                        </button>
                      ))}
                    </div>
                  ) : null}
              ...
              <div className="grid gap-4 p-4 sm:gap-6 sm:p-6 md:grid-cols-3 md:p-8">
                <div className="md:col-span-2">
                  <div className="mb-3 flex flex-wrap items-center gap-2 text-xs sm:mb-4 sm:gap-3 sm:text-sm">
                    <span className="flex items-center gap-1"><Star className="h-3.5 w-3.5 fill-yellow-400 text-yellow-400 sm:h-4 sm:w-4" /><span className="font-semibold">{tmdbRating}</span><span className="text-muted-foreground">TMDB</span></span>
                    {avgUserRating && <span className="flex items-center gap-1 text-primary"><Star className="h-3.5 w-3.5 fill-primary text-primary sm:h-4 sm:w-4" /><span className="font-semibold">{avgUserRating}</span><span className="text-muted-foreground">User</span></span>}
                  ...
                  {detail.genres && detail.genres.length > 0 && (<div className="mb-3 flex flex-wrap gap-1.5 sm:mb-4">{detail.genres.map((g) => <Badge key={g.id} variant="outline" className="text-xs">{g.name}</Badge>)}</div>)}
                  ...
                  {cast.length > 0 && (
                    <div className="mt-4 sm:mt-6">
                      ...
                      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-3 lg:grid-cols-4">
                        {cast.map((p) => (<div key={p.id} className="flex items-center gap-2"><div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-full bg-muted sm:h-12 sm:w-12">{p.profile_path && <Image src={getImageUrl(p.profile_path, "w185")} alt={p.name} fill sizes="48px" className="object-cover" unoptimized />}</div><div className="min-w-0 flex-1"><p className="truncate text-xs font-medium sm:text-sm">{p.name}</p><p className="truncate text-[10px] text-muted-foreground sm:text-xs">{p.character}</p></div></div>))}
                      </div>
                    </div>
                  )}

                  <div className="mt-6 border-t border-border pt-4">
                    <h3 className="mb-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground sm:text-sm">Rate This {isTV ? "Series" : "Movie"}</h3>
                    <div className="flex items-center gap-1">
                      {[1,2,3,4,5].map((star) => { const value = star * 2; const isActive = (hoverRating || userRating?.rating || 0) >= value; return (<button key={star} onClick={() => handleRate(value)} onMouseEnter={() => setHoverRating(value)} onMouseLeave={() => setHoverRating(0)} disabled={ratingLoading} className="transition-transform hover:scale-110 disabled:opacity-50"><Star className={isActive ? "h-6 w-6 fill-yellow-400 text-yellow-400 sm:h-7 sm:w-7" : "h-6 w-6 text-muted-foreground sm:h-7 sm:w-7"} /></button>); })}
                      <span className="ml-3 text-sm font-medium">{ratingLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : userRating ? <span className="text-primary">{userRating.rating}/10</span> : <span className="text-muted-foreground">Click to rate</span>}</span>
                    </div>
                ...
                <div className="space-y-3 sm:space-y-4">
                  {(director || creator) && <div><h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{isTV ? "Creator" : "Director"}</h4><p className="mt-0.5 text-xs sm:text-sm">{director || creator}</p></div>}
                  {isTV && detail.number_of_seasons && <div className="flex gap-4"><div><h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Seasons</h4><p className="mt-0.5 text-xs sm:text-sm">{detail.number_of_seasons}</p></div><div><h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Episodes</h4><p className="mt-0.5 text-xs sm:text-sm">{detail.number_of_episodes}</p></div></div>}
                  {detail.spoken_languages && detail.spoken_languages.length > 0 && <div><h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Languages</h4><p className="mt-0.5 text-xs sm:text-sm">{detail.spoken_languages.map((l) => l.english_name).join(", ")}</p></div>}
                  {detail.production_companies && detail.production_companies.length > 0 && <div><h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Production</h4><p className="mt-0.5 text-xs sm:text-sm">{detail.production_companies.map((c) => c.name).join(", ")}</p></div>}
                </div>
              </div>

              {/* More Like This - horizontal scroll with touch support */}
              {similar.length > 0 && (
                <div className="border-t border-border py-4 sm:py-6">
                  ...
                  <div className="flex gap-3 overflow-x-auto px-4 pb-2 sm:px-6 lg:px-8" style={{ scrollbarWidth: "none", msOverflowStyle: "none", touchAction: "pan-x" }}>
              ...
              {/* Comments */}
              <div className="border-t border-border px-4 py-4 pb-16 sm:px-6 sm:py-6 md:px-8">
                ...
                <div className="mb-4 flex gap-2">
                  <Avatar className="h-8 w-8 shrink-0"><AvatarImage src={session?.user?.image || undefined} /><AvatarFallback className="bg-primary/20 text-xs text-primary">{session?.user?.name?.[0]?.toUpperCase() || "U"}</AvatarFallback></Avatar>
  ...
  return (
    <div className={level > 0 ? "ml-8 border-l border-border pl-3" : ""}>
      <div className="flex gap-2">
        <Avatar className="h-8 w-8 shrink-0"><AvatarImage src={comment.userImage || undefined} /><AvatarFallback className="bg-primary/20 text-xs text-primary">{initial}</AvatarFallback></Avatar>
        <div className="min-w-0 flex-1">
          <div className="rounded-lg bg-muted/40 px-3 py-2"><div className="flex items-center justify-between gap-2"><span className="text-xs font-semibold">{comment.userName || "Anonymous"}</span><span className="text-[10px] text-muted-foreground">{timeAgo}</span></div><p className="mt-1 text-xs leading-relaxed sm:text-sm">{comment.content}</p></div>
          <div className="mt-1 flex items-center gap-3 px-1"><button onClick={() => onReply(comment.id)} className="flex items-center gap-1 text-[10px] text-muted-foreground hover:text-primary"><CornerDownRight className="h-3 w-3" />Reply</button>{isOwner && <button onClick={() => onDelete(comment.id)} className="flex items-center gap-1 text-[10px] text-muted-foreground hover:text-destructive"><Trash2 className="h-3 w-3" />Delete</button>}</div>
          {replyTo === comment.id && <div className="mt-2 flex gap-2"><textarea value={replyText} onChange={(e) => setReplyText(e.target.value)} placeholder={`Reply to ${comment.userName || "user"}...`} className="flex-1 resize-none rounded-lg border border-border bg-transparent px-3 py-2 text-xs outline-none focus:border-primary" rows={2} maxLength={2000} autoFocus /><Button size="sm" onClick={() => onPostReply(comment.id)} disabled={!replyText.trim() || commentLoading} className="gap-1 self-end">{commentLoading ? <Loader2 className="h-3 w-3 animate-spin" /> : <Send className="h-3 w-3" />}</Button></div>}
        </div>
      </div>
      {comment.replies && comment.replies.length > 0 && <div className="mt-3 space-y-3">{comment.replies.map((r: any) => <CommentNode key={r.id} comment={r} currentUserId={currentUserId} onReply={onReply} replyTo={replyTo} replyText={replyText} setReplyText={setReplyText} onPostReply={onPostReply} onDelete={onDelete} commentLoading={commentLoading} level={level + 1} />)}</div>}
    </div>
  );
}
