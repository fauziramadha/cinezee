...
export function AuthModal() {
  ...
  // Handle Google OAuth login via direct form submission (bypass SW/fetch issues)
  const handleGoogleLogin = () => {
    // Buat form hidden untuk POST ke /api/auth/signin/google
    // Ini cara paling reliable di Cloudflare Workers + PWA
    ...
    // Kita butuh csrf token, fetch dulu
    fetch("/api/auth/csrf")
      .then((res) => res.json())
      .then((data) => {
        ...
        form.submit(); // Browser akan handle redirect ke Google
      })
      .catch((err) => {
        console.error("[Auth] Gagal ambil CSRF token:", err);
        toast.error("Gagal memulai login Google. Coba lagi.");
      });
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // Reload untuk update session di header
