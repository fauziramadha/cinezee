...
import { Play, ChevronLeft, ChevronRight, Info, Tv } from "lucide-react";
...
export function DrakorHero({ drakors }: DrakorHeroProps) {
  ...
  if (!drakors || drakors.length === 0) return null;

  const heroItems = drakors.slice(0, 5);

  const handleWatch = (item: any) => {
    const slug = (item.id || item.slug || "").toString().replace(/\/+$/, "").trim();
    if (!slug) return;
    router.push(`/drakor/${slug}`);
  };
      ...
      <div ref={emblaRef} className="h-full overflow-hidden">
        <div className="flex h-full">
          {heroItems.map((item, idx) => {
            ...
            const poster = item.imageUrl || item.poster || item.thumbnail || null;
            const status = item.status || "Ongoing";
            const episode = item.episode || item.current_episode || "";
            const slug = (item.id || item.slug || "").toString().replace(/\/+$/, "").trim();
